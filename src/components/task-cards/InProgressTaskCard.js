  ...
  return (
    <>
      <div className="card pb-2" style={{ width: "100%" }}>
        <h5 className="card-title bg-warning p-2 text-light rounded-top bold">In Progress</h5>
        {tasks && tasks.filter(el => el.status === 'inProgress').length === 0 && <h6 className='text-danger mt-2 height-230 dfjcac'>No Tasks</h6>}
        {tasks==='' && <div className='dfjcac height-230'><Spinner height={35} width={35} /></div>}
        {tasks && tasks.filter(el => el.status === 'inProgress').map((task, index) => {
          return (
          <div key={index} className="mx-2 mb-2 bg-secondary-light">
            ...
            <div className='row p-2 mt-2'>
              <div className="col dfjsac">
                <span className='w-50 bg-blue text-light card-btn rounded p-1'>In Progress</span>
              </div>
            </div>
          </div>
          )
